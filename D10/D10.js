...
/* ESERCIZIO E
  ...
  precedentemente creato un array chiamato "skills", contenente i linguaggi di programmazione che conosci.
*/
...
/* ESERCIZIO 6
  Crea una funzione chiamata "isThisAnEmail"
   che riceve una stringa come parametro e ritorna true se la stringa è un valido indirizzo email.
*/

function isThisAnEmail(str) {
  const str1 = str.search(/^((?!\.)[\w\-_.]*[^.])(@\w+)(\.\w+(\.\w+)?[^.\W])$/gm);
  if (str1 === 0) {
    return true
  } else {
    return false
...
console.log(isThisAnEmail("nicolavettone57@gmail.com"))
...
function whatDayIsIt() {
  let today = now.getDay()   // provando a farlo con switch mi dava un errore.
  if(today === 1){
    return "Lunedì"
   }
   if(today === 2){
...
function rollTheDices(num) {
  ...
  for (let i = 0; i < num; i++) {
    let num1 = dice()
   // numeri.values.push(num1)  somma non funziona
    //numeri.sum += num1
  }
...
/* ESERCIZIO 9
  Scrivi una funzione chiamata "howManyDays" 
  che riceve una data come parametro e ritorna il numero di giorni trascorsi da tale data.
*/
let dataAttuale = new Date();

function quantiGiorni(YYYY_DD_MM) {
  let data1 = new Date(YYYY_DD_MM)
  let tempoTrascorso = dataAttuale - data1
  let giorniTrascorsi = Math.floor(tempoTrascorso / (1000 * 60 * 60 * 24));
  
  return giorniTrascorsi;
}
console.log(quantiGiorni("1999-10-04"));
...
let oggi = now.getDate()
function isTodayMyBirthday(giorno , mese1) {
  let giornoCompleanno = giorno
  let meseCompleanno = mese1
  if (giornoCompleanno === oggi && meseCompleanno === mese + 1 ){
    return true
  } else{
    return false
  }
}
console.log(isTodayMyBirthday(4 , 10))
...
function deleteProp(proprietà, obj) {
  obj = {
  name : "Nicola",
  lastname : "Vettone",
  age : 25
...
function onlyTheYears() {
  let annoFilm = movies.map((element)=> element = element.Year)
...
function sumAllTheYears() {
  let sommaAnnni = movies.reduce((acc,val) => acc += parseInt(val.Year),0)

console.log(sommaAnnni)
}
sumAllTheYears()
